...
export default function RelEditModal({ rel, onClose, onUpdated, suggestedKeys = [] }: Props) {
  ...
  const existingKeys = Object.keys(rel.properties);
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      await SupplyChainApi.updateRelationship(rel.elementId, {
    ...
    >
      <div className="w-full max-w-2xl card-pad mt-10" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-base font-semibold text-slate-900">Gestionar propiedades de la relación</h2>
            <p className="text-xs text-slate-500">
              (:{rel.type}) · {rel.startId ?? "?"} → {rel.endId ?? "?"}
            </p>
          </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {existingKeys.map((k) => (
                <span key={k} className="rounded bg-white border border-slate-200 px-2 py-0.5 text-[11px] text-slate-700">
                  ...
                  {String(rel.properties[k] ?? "—").slice(0, 40)}
                </span>
              ))}
            </div>
          ) : (
            <span className="text-[11px] text-slate-400">Sin propiedades aún.</span>
          )}
        </div>
